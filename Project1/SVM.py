...
for train_index, val_index in kf.split(df['raw'], df['label']):
    ...
    # model = SVC(kernel='rbf', C=10, gamma=0.0001) # 使用高斯核函数
...
print(f"SVM准确率: {avg_accuracy}")
print(f"SVM精确率: {avg_precision}")
print(f"SVM召回率: {avg_recall}")
print(f"SVM F1分数: {avg_f1}")
...
out = open('SVM results.txt', 'w', encoding='utf-8')
out.write('id, pred\n')
